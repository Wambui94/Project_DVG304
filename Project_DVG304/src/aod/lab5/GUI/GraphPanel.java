...
/**
 * Panel responsible for drawing the map, vertices, and edges.
 *
 * This class handles the graphical visualization of the graph. It draws the
 * Sweden map as a background, displays vertices and edges, supports zooming and
 * panning, and highlights shortest paths.
 *
...
public class GraphPanel<T> extends JPanel {
	...
	public GraphPanel(Graph<T> graph) {
		...
		try {
			swedenMap = ImageIO.read(getClass().getResource("/map-of-sweden-with-cities.png"));
	...
	private void drawMapBackground(Graphics2D g2) {
		if (swedenMap != null) {
			g2.drawImage(swedenMap, 0, 0, GraphViewer.MAP_WIDTH, GraphViewer.MAP_HEIGHT, null);
		}
	}
	...
	private void drawEdge(Graphics2D g2, Edge<T> edge) {
		...
		boolean isShortestPath = edge.getColor() != null && edge.getColor().equals(Color.RED);
	...
	private void drawVertex(Graphics2D g2, Vertex<T> v) {
		...
		if (zoom >= 6.0) {
			...
			g2.drawString(label, x - textWidth / 2, y + textHeight / 3);
		}
	}
